...
void io_battle(character *aggressor, character *defender)
{
  ...
  refresh();
  getch();
  // If null (the agressor is the pc)
  if (!(n = dynamic_cast<NPC *>(aggressor))) {
    // Cast the defender to the npc (only if the aggressor isn't casted -> dynamic_cast)
...
void io_fly(pair_t dest) {
  ...
  // PC is no longer there, and I don't want it there if I come back
  world.cur_map->cmap[world.pc.pos[dim_y]][world.pc.pos[dim_x]] = NULL;

  // Turn on curser and echo so the user can see their input
  curs_set(1);
  echo();

  // Prompt the user for input and read it in
  mvprintw(0, 0, "Where would you like to fly to? (x, y): ");
  mvscanw(0, 40, "%d, %d", &world_x, &world_y);

  // Turn these back off since we don't want them on in general
  noecho();
  curs_set(0);

  // Account for the fact that the user's coordinates are not the proper world coordinates
  world_x += 200;
  world_y += 200;

  world.cur_idx[dim_x] = world_x;
  world.cur_idx[dim_y] = world_y;
