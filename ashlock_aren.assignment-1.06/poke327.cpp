...
void game_loop()
{
  ...
  while (!world.quit) {
    ...
    // If it is the pc (pointer is not null after dynamic cast) and it moved to a gate, then it is moving to another map
    if (p && (d[dim_x] == 0 || d[dim_x] == MAP_X - 1 || d[dim_y] == 0 || d[dim_y] == MAP_Y - 1)) {
      // Have to do this here since my original idea would've set the new spot to null, not the old one (in line 1125)
      if (d[dim_x] == 0) {
        world.cur_idx[dim_x]--;
      }
      else if (d[dim_y] == 0) {
      ...
      // Since we are moving to another map, the "new_map" will update the pc position and that's essentially our move
      d[dim_x] = c->pos[dim_x];
      d[dim_y] = c->pos[dim_y];
    }
