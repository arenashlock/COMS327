...
void fill_terrain_quad(struct single_map *map, int t_x, int t_y, int quad_x_mult, int quad_y_mult, char t_char){
    int r, c;
        ...
        if(!((t_y + (r * quad_y_mult)) <= 0 || (t_y + (r * quad_y_mult)) >= 20)){
            for(c = 0; c < t_width; c++){
                if(!((t_x + (c * quad_x_mult)) <= 0 || (t_x + (c * quad_x_mult)) >= 79)){
                    ...
                    map->terrain[t_y + (r * quad_y_mult)][t_x + (c * quad_x_mult)] = t_char;
                }
            }
        }
    }
...
void gen_terrain(struct single_map *map){
    ...
    for(t_type = 1; t_type < 8; t_type++){
        ...
        // Fill in Quadrant 1
        fill_terrain_quad(map, t_x, t_y, -1, -1, t_char);
        // Fill in Quadrant 2
        fill_terrain_quad(map, t_x, t_y, 1, -1, t_char);
        // Fill in Quadrant 3
        fill_terrain_quad(map, t_x, t_y, -1, 1, t_char);
...
void gen_N_to_S(struct single_map *map){
...
void gen_W_to_E(struct single_map *map){
    int r, c, smaller_gate_val, bigger_gate_val;
    // Want a little space between the path and the border for a potential Pokemon Center and Pokemart
...
void gen_buildings(struct single_map *map){
    ...
    if(map->terrain[spawn_y][spawn_x] != '#'){
        gen_buildings(map);
    }
        ...
        for(r = 1; r <= 2; r++){
            for(c = 0; c <= 1; c++){
                map->terrain[spawn_y + r][spawn_x + c] = 'M';
                map->terrain[spawn_y - r][spawn_x + c] = 'C';
            }
        }
    }
