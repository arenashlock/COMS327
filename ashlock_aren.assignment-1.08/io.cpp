...
void io_pc_choose_pokemon() {
  // Generate 3 Pokemon at level 1
  Pokemon *pokemon_1 = new Pokemon(1);
  Pokemon *pokemon_2 = new Pokemon(1);
  Pokemon *pokemon_3 = new Pokemon(1);

  // Make the screen for the user choice
  mvprintw(0, 0, "Choose your first Pokemon: ");
  mvprintw(1, 3, "1: %s", pokemon_1->get_species());
  mvprintw(2, 3, "2: %s", pokemon_2->get_species());
  mvprintw(3, 3, "3: %s", pokemon_3->get_species());
  ...
  int i;
  // Clear the slots holding each Pokemon
  for (i = 0; i < 6; i++)
  {
  ...
  switch (input) {
    case '1':
      world.pc.char_pokemon[0] = pokemon_1;
      // Don't need the other 2 Pokemon we generated
      delete pokemon_2;
      delete pokemon_3;
      break;
    case '2':
      world.pc.char_pokemon[0] = pokemon_2;
      // Don't need the other 2 Pokemon we generated
      delete pokemon_1;
      delete pokemon_3;
      break;
    case '3':
      world.pc.char_pokemon[0] = pokemon_3;
      // Don't need the other 2 Pokemon we generated
      delete pokemon_1;
      delete pokemon_2;
      break;
...
void io_trainer_pokemon(npc *n) {
  ...
  for(i = 0; i < n->party_size; i++) {
    // Print the stats of the Pokemon
    clear();

    mvprintw(0, 0, "This trainer has %d Pokemon!", n->party_size);
