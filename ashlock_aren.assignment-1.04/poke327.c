...
typedef struct pc {
  // remove pair_t pos due to it being in character_t
} pc_t;
...
static int32_t character_turns_cmp(const void *key, const void *with) {
  return ((character_t *) key)->next_turn - ((character_t *) with)->next_turn;
}

// We want to be able to delete chracters, so we need to compare the address of the pointer to the different kinds of characters
...
static int new_map()
{
  ...
  heap_init(&world.cur_map->character_turns, character_turns_cmp, delete_character);
...
void spawn_hiker()
{
  ...
  do
  {
    y = (rand() % (MAP_Y - 2)) + 1;
    x = (rand() % (MAP_X - 2)) + 1;
  } while (world.hiker_dist[y][x] == INT_MAX || world.cur_map->cmap[y][x]);
    // Need the distance map for a hiker since there is a chance the npc can spawn in a valid spot, but can't move anywhere

  // Initialize the hiker
  // malloc space
  ...
  character->npc->cost_type = char_hiker;
  // Attribute is not applicable to hikers
...
void spawn_rival()
{
  ...
  do
  {
    y = (rand() % (MAP_Y - 2)) + 1;
    x = (rand() % (MAP_X - 2)) + 1;
  } while (world.rival_dist[y][x] == INT_MAX || world.cur_map->cmap[y][x]);
  // Need the distance map for a rival since there is a chance the npc can spawn in a valid spot, but can't move anywhere

  // Initialize the rival
  // malloc space
  character = malloc(sizeof(*character));
  character->npc = malloc(sizeof(*character->npc));
  // character attributes
  ...
  character->npc->cost_type = char_rival;
  // Attribute is not applicable to rivals
  character->npc->move[dim_x] = 0;
  character->npc->move[dim_y] = 0;
...
void spawn_other()
{
  ...
  // Since the movement costs for a rivel and other characters are the same, we can utilize the rival distance map
  // Need the distance map for a rival since there is a chance the npc can spawn in a valid spot, but can't move anywhere
...
void spawn_npcs(int total_npcs){
  ...
  if(total_npcs < 2){
    ...
    // Guaranteed to spawn a hiker and a rival
    spawn_hiker();
    spawn_rival();
    int extra_npcs = total_npcs - 2;

    for(i = 0; i < extra_npcs; i++){
      // For the rest of the npcs, I will give a 10% chance of either a hiker or a rival to spawn
      switch(rand() % 10){
        case 0:
          spawn_hiker();
          break;
...
void move_hiker(character_t *hiker){
  pair_t min_movement;
  // Initialization doesn't matter, but INT_MAX will always be larger than the min
  int min = INT_MAX;
  int r, c;

  // Since we are moving the npc, we need to set the spot in cmap to NULL
...
void move_pacer(character_t *pacer){
  ...
  while(world.rival_dist[pacer->pos[dim_y] + pacer->npc->move[dim_y]][pacer->pos[dim_x] + pacer->npc->move[dim_x]] == INT_MAX){
...
void move_explorer(character_t *explorer){
  // Since we are moving the npc, we need to set the spot in cmap to NULL
  world.cur_map->cmap[explorer->pos[dim_y]][explorer->pos[dim_x]] = NULL;

  while(world.rival_dist[explorer->pos[dim_y] + explorer->npc->move[dim_y]][explorer->pos[dim_x] + explorer->npc->move[dim_x]] == INT_MAX){
    switch(rand() % 4){
      // North
      case 0:
        explorer->npc->move[dim_x] = 0;
        explorer->npc->move[dim_y] = -1;
        break;
      // South
      case 1:
        explorer->npc->move[dim_x] = 0;
      ...
      // East
      case 2:
        explorer->npc->move[dim_x] = 1;
        explorer->npc->move[dim_y] = 0;
        break;
      // West
      default:
        explorer->npc->move[dim_x] = -1;
  ...
  explorer->pos[dim_y] = explorer->pos[dim_y] + explorer->npc->move[dim_y];
  explorer->pos[dim_x] = explorer->pos[dim_x] + explorer->npc->move[dim_x];
  world.cur_map->cmap[explorer->pos[dim_y]][explorer->pos[dim_x]] = explorer;
}

void move_npc(){
  character_t *to_move;
  
  if((to_move = heap_remove_min(&world.cur_map->character_turns))->char_type != char_pc){
    switch (to_move->char_type){
      case char_hiker:
        move_hiker(to_move);
        break;
      case char_rival:
      ...
      case char_pacer:
        ...
        break;
      case char_wanderer:
        move_wanderer(to_move);
        break;
      case char_explorer:
        move_explorer(to_move);
        break;
      // Sentry won't move, so it falls in the default
      default:
        break;
    }
    to_move->next_turn += move_cost[to_move->npc->cost_type][world.cur_map->map[to_move->pos[dim_y]][to_move->pos[dim_x]]];
  }

  else{
    to_move->next_turn += move_cost[char_pc][world.cur_map->map[to_move->pos[dim_y]][to_move->pos[dim_x]]];
  }

  heap_insert(&world.cur_map->character_turns, to_move);
}
...
int main(int argc, char *argv[])
{
  ...
  // I figure generating 4-10 total npcs is valid
  int total_npcs;
  if(argc < 2){
    total_npcs = (rand() % 7) + 4;
  }
  else{
    total_npcs = (int) argv[1][2] - '0';
  }
